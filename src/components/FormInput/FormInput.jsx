...
function FormInput({
  ...
  const validateValue = evt => {
    if (typeof checkValue === 'function') {
      if (checkValue(evt.target.value)) {
        ...
        setIsValid(false);
        setErrMessage(evt.target.validationMessage || 'Введены некорректные данные');
      }
    } else {
  ...
  return (
    <label className={`${className}__field`} htmlFor={htmlFor}>
      ...
      <input
        className={`${className}__input ${isValid ? '' : `${className}__input_error`}`}
        ...
        required
      />
      <span className={`${className}__error ${!errMessage ? '' : `${className}__error_active`}`}>
        {errMessage}
      </span>
    </label>
  );
